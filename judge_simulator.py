...
LLM_API_KEY = os.environ.get("GROQ_API_KEY", "")  # <-- PUT YOUR API KEY HERE OR SET ENV VAR
...
class JudgeSimulator:
    ...
    def _auto_reply(self) -> bool:
        ...
        conv_auto_id = f"conv_auto_{int(time.time())}"
        for i in range(1, 5):
            print_info(f"Turn {i}: Sending auto-reply...")
            data, err, _ = self.client.reply(conv_auto_id, mid, auto_msg, i + 1)
